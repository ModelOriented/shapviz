...
SHAP values [@lundberg2017] are an ingenious way to study black box models. They decompose - as fair as possible - predictions into additive feature contributions. Crunching SHAP values requires clever algorithms by clever people. Analyzing them, however, is super easy with the right visualizations. The package `shapviz` offers the latter:
...
for XGBoost and LightGBM allows to select the categery of interest and work with its predicted (logit) probabilities, simply by passing `which_class` in the constructor.
