...
## User-visible changes (non-breaking)

- `sv_importance()` now shows a bar plot by default instead of a beeswarm plot. Use `kind = "beeswarm"` to get a beeswarm plot.
- The bar plot of `sv_importance()` does not print numeric SHAP feature importances anymore. Use `show_numbers = TRUE` to get them back. Furthermore, the numbers are now printed on top of the bars instead on their bottom.
...
- `sv_importance()` received an argument `show_numbers` to add or hide SHAP feature importance values for all plot types.
...
- The argument `format_fun` in `sv_importance()` now uses a better aligned number formatter.
