...
#'
#' Function used to collapse SHAP values. A typical application is when
#' the matrix of SHAP values is generated by a model with explicit one-hot
#' encoded variables, while the explanation should happen at the level of the original
#' character vector x. Due to additivity, the SHAP values of all one-hot encoded
#' variables of x can be added up in a row-wise fashion.
#' Collapsing always means summation.
#' Currently, only SHAP objects of type "matrix" are supported.
#'
#' @param object An object containing SHAP values.
#' @param collapse A named list of character vectors explaining how to collapse SHAP values.
#' In case \code{object} is a matrix, the names of \code{collapse} specify the resulting
#' collapsed column names, while the values specify which columns in \code{object} to collapse.
#' @param ... Currently unused.
